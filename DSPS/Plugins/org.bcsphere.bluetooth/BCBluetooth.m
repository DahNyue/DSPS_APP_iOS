    ...
    if (peripheral) {
        if (IS_IOS_VERSION_7_HIGHER) {
            if (peripheral.state == CBPeripheralStateConnected) {
                [self callConnectCallback:deviceAddress callbackId:command.callbackId isKeepCallback:FALSE];
...
- (void)callConnectCallback:(NSString *)deviceID callbackId:(NSString *)callback isKeepCallback:(BOOL)isKeepCallback{
...
- (void)disconnect:(CDVInvokedUrlCommand*)command{
    ...
    CBPeripheral *peripheral = [self getPeripheral:deviceAddress];
    if (peripheral) {
        // Check if the connection is active or pending
        if (peripheral.state == CBPeripheralStateConnected || [self.callbacks valueForKey:[NSString stringWithFormat:@"connect:%@",deviceAddress]]) {
            ...
            NSLog(@"Already disconnected from %@", deviceAddress);
            [self callConnectCallback:deviceAddress callbackId:command.callbackId isKeepCallback:FALSE];
        }
    }else{
        [self error:command.callbackId];
...
- (void)getServices:(CDVInvokedUrlCommand*)command{
    ...
    if (peripheral) {
        if (peripheral.services.count > 0){
            ...
            NSString* key = [NSString stringWithFormat:@"getServices:%@",deviceAddress];
            if ([self operationPending:key command:command])
                return;
            [self.callbacks setValue:command.callbackId forKey:key];
...
- (void)getCharacteristics:(CDVInvokedUrlCommand*)command{
    ...
    if (peripheral && serviceIndex && (peripheral.services.count > serviceIndex.intValue)) {
        ...
        if (service.characteristics.count > 0) {
            ...
            NSString* key = [NSString stringWithFormat:@"getCharacteristics:%@:%@",serviceIndex,deviceAddress];
            if ([self operationPending:key command:command])
                return;
            [self.callbacks setValue:command.callbackId forKey:key];
            peripheral.delegate = self;
    ...
    if (peripheral && serviceIndex && characteristicIndex && (serviceIndex.intValue < peripheral.services.count)){
        ...
        if ((characteristicIndex.intValue < service.characteristics.count)) {
            ...
            if (characteristic.descriptors.count > 0) {
                ...
                NSString* key = [NSString stringWithFormat:@"getDescriptors:%@:%@:%@",characteristicIndex,serviceIndex,deviceAddress];
                if ([self operationPending:key command:command])
                    return;
                [self.callbacks setValue:command.callbackId forKey:key];
                peripheral.delegate = self;
...
- (void)getRSSI:(CDVInvokedUrlCommand*)command{
    ...
    CBPeripheral *peripheral = [self getPeripheral:deviceAddress];
    if (peripheral) {
        NSString* key = [NSString stringWithFormat:@"getRssi:%@",deviceAddress];
        if ([self operationPending:key command:command])
            return;
        [self.callbacks setValue:command.callbackId forKey:key];
        peripheral.delegate = self;
        ...
        if (service.characteristics.count > [characteristicIndex intValue]) {
            ...
            if (descriptorIndex && descriptorIndex.length > 0){
                ...
                if (descriptorIndex.intValue < characteristic.descriptors.count) {
                    NSString* key = [NSString stringWithFormat:@"writeDescriptor:%@:%@:%@:%@",
                                              descriptorIndex,characteristicIndex,serviceIndex,deviceAddress];
                    if ([self operationPending:key command:command])
                        return;
                    [self.callbacks setValue:command.callbackId forKey:key];
                    peripheral.delegate = self;
                    CBDescriptor *descriptor = [characteristic.descriptors objectAtIndex: [descriptorIndex intValue] ];
                    [peripheral writeValue:data forDescriptor:descriptor];
                    BCLOG_DATA(data,@"writeDescriptor",deviceAddress, [self CBUUIDToString:descriptor.UUID])
                }else{
                    [self error:command.callbackId];
                }
            }else{
                ...
                if (characteristic.properties & CBCharacteristicPropertyWriteWithoutResponse) {
                    // Write without response. Call callback immediately as there in no callback from the stack.
                    ...
                    NSString* key = [NSString stringWithFormat:@"writeCharacteristic:%@:%@:%@",
                                              characteristicIndex,serviceIndex,deviceAddress];
                    if ([self operationPending:key command:command])
                        return;
                    [self.callbacks setValue:command.callbackId forKey:key];
        ...
        if (characteristicIndex.intValue < service.characteristics.count){
            ...
            if (descriptorIndex && descriptorIndex.length > 0){
                ...
                if (characteristic.descriptors.count > [descriptorIndex intValue]) {
                    NSString* key = [NSString stringWithFormat:@"readDescriptor:%@:%@:%@:%@",
                                              descriptorIndex,characteristicIndex,serviceIndex,deviceAddress];
                    if ([self operationPending:key command:command])
                        return;
                    [self.callbacks setValue:command.callbackId forKey:key];
                ...
                // Read characteristic
                NSString* key = [NSString stringWithFormat:@"readCharacteristic:%@:%@:%@",
                                          characteristicIndex,serviceIndex,deviceAddress];
                if ([self operationPending:key command:command])
                    return;
                [self.callbacks setValue:command.callbackId forKey:key];
...
- (void)setNotification:(CDVInvokedUrlCommand*)command{
    ...
    if (peripheral && serviceIndex && characteristicIndex && (serviceIndex.intValue < peripheral.services.count)) {
        ...
        if (characteristicIndex.intValue < service.characteristics.count) {
            CBCharacteristic *characteristic = [service.characteristics objectAtIndex:[characteristicIndex intValue]];
            NSString* setNotificationKey = [NSString stringWithFormat:@"setNotification:%@:%@:%@",characteristicIndex,serviceIndex,deviceAddress];
            NSString* enableKey = [NSString stringWithFormat:@"notificationEnable:%@:%@:%@",characteristicIndex,serviceIndex,deviceAddress];
            if ([self operationPending:setNotificationKey command:command])
                return;
            [self.callbacks setValue:command.callbackId forKey:setNotificationKey];
            [self.callbacks setValue:enable forKey:enableKey];
            peripheral.delegate = self;
            if ([enable boolValue]) {
                [self.callbacks setValue:command.callbackId forKey:[NSString stringWithFormat:@"notification:%@:%@:%@",
                                                                             characteristicIndex,serviceIndex,deviceAddress]];
...
- (void)centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral
{
    ...
    NSString *connectCallback = [self getCompletionCallback:[NSString stringWithFormat:@"connect:%@",deviceAddress]];
    [self removePendingOperations:deviceAddress];
    [self callConnectCallback:deviceAddress callbackId:connectCallback isKeepCallback:FALSE];
}
...
- (void)centralManager:(CBCentralManager *)central didFailToConnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error
{
    ...
    NSString *deviceAddress = [self getPeripheralUUID:peripheral];
    NSString *connectCallback = [self getCompletionCallback:[NSString stringWithFormat:@"connect:%@",deviceAddress]];
    NSString *disconnectCallback = [self getCompletionCallback:[NSString stringWithFormat:@"disconnect:%@",deviceAddress]];
    [self error:connectCallback];
    // Check for pending disconnect
    if (disconnectCallback) {
        [self callConnectCallback:deviceAddress callbackId:disconnectCallback isKeepCallback:FALSE];
    }
}
...
- (void)centralManager:(CBCentralManager *)central didDisconnectPeripheral:(CBPeripheral *)aPeripheral error:(NSError *)error
{
    ...
    if (disconnectCallback) {
        ...
        [self callConnectCallback:deviceAddress callbackId:[self.callbacks valueForKey:EVENT_DISCONNECT] isKeepCallback:TRUE];
    }
}
...
- (void)peripheralDidUpdateRSSI:(CBPeripheral *)peripheral error:(NSError *)error{
    BCLOG_FUNC(GAP_MODUAL)
    NSString *callback = [self getCompletionCallback:[NSString stringWithFormat:@"getRssi:%@",[self getPeripheralUUID:peripheral]]];
    if (!error) {
        NSMutableDictionary *callbackInfo = [[NSMutableDictionary alloc] init];
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverServices:(NSError *)error {
    BCLOG_FUNC(GAP_MODUAL)
    NSString *deviceAddress = [self getPeripheralUUID:peripheral];
    NSString *callback = [self getCompletionCallback:[NSString stringWithFormat:@"getServices:%@",deviceAddress]];
    if (!error) {
        NSMutableDictionary *callbackInfo = [self storeServiceInfo:peripheral];
        CDVPluginResult* result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:callbackInfo];
        [self.commandDelegate sendPluginResult:result callbackId:callback];
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error{
    BCLOG_FUNC(GATT_MODUAL)
    NSString *deviceAddress = [self getPeripheralUUID:peripheral];
    NSString *callback = [self getCompletionCallback:[NSString stringWithFormat:@"getCharacteristics:%d:%@",
                                                               [self getServiceIndex:peripheral service:service],
                                                               deviceAddress]];
    if (!error) {
        NSMutableDictionary *callbackInfo = [self storeChatacteristicInfo:peripheral service:service];
        CDVPluginResult* result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:callbackInfo];
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverDescriptorsForCharacteristic:
    ...
    BCLOG_FUNC(GATT_MODUAL)
    NSString *deviceAddress = [self getPeripheralUUID:peripheral];
    NSString *callback = [self getCompletionCallback:[NSString stringWithFormat:@"getDescriptors:%d:%d:%@",
                                                               [self getCharacterIndex:characteristic.service character:characteristic],
                                                               [self getServiceIndex:peripheral service:characteristic.service],
                                                               deviceAddress]];
    if (!error) {
        NSMutableDictionary *callbackInfo = [self storeDescriptorInfo:peripheral characteristic:characteristic];
        CDVPluginResult* result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:callbackInfo];
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    NSString *readCallback = [self getCompletionCallback:[NSString stringWithFormat:@"readCharacteristic:%@:%@:%@",
                                                                   characteristicIndex,serviceIndex,deviceAddress]];
    if (!error) {
        // Don't remove notification callback
        NSString *notificationCallback = [self.callbacks valueForKey:[NSString stringWithFormat:@"notification:%@:%@:%@",
                                                                               characteristicIndex,serviceIndex,deviceAddress]];
        ...
        if (notificationCallback) {
            ...
            CDVPluginResult* result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:callbackInfo];
            [result setKeepCallbackAsBool:TRUE];
            [self.commandDelegate sendPluginResult:result callbackId:notificationCallback];
        }
        if (readCallback) {
            NSMutableDictionary *callbackInfo = [[NSMutableDictionary alloc] init];
            NSString *value = [self encodeBase64:[characteristic value]];
            ...
            [self.commandDelegate sendPluginResult:result callbackId:readCallback];
        }
    }else{
        if (readCallback) {
            [self error:readCallback];
        }
    }
}
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForDescriptor:(CBDescriptor *)descriptor error:(NSError *)error{
    BCLOG_FUNC(GATT_MODUAL)
    NSString *callback = [self getCompletionCallback:[NSString stringWithFormat:@"readDescriptor:%d:%d:%d:%@",
                                                               [self getDescriptorIndex:descriptor.characteristic descriptor:descriptor],
                                                               [self getCharacterIndex:descriptor.characteristic.service character:descriptor.characteristic],
                                                               [self getServiceIndex:peripheral service:descriptor.characteristic.service],
                                                               [self getPeripheralUUID:peripheral]]];
    if (!error) {
        NSMutableDictionary *callbackInfo = [[NSMutableDictionary alloc] init];
        NSString *date = [NSString stringWithFormat:@"%@",[self getTime]];
...
{
    BCLOG_FUNC(GATT_MODUAL)
    NSString *callback = [self getCompletionCallback:[NSString stringWithFormat:@"writeCharacteristic:%d:%d:%@",
                                                               [self getCharacterIndex:characteristic.service character:characteristic],
                                                               [self getServiceIndex:peripheral service:characteristic.service],
                                                               [self getPeripheralUUID:peripheral]]];
    // Workaround for iOS 10 crash
    if (!callback)
        return;
    if (!error) {
        NSMutableDictionary *callbackInfo = [[NSMutableDictionary alloc] init];
...
- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForDescriptor:(CBDescriptor *)descriptor error:(NSError *)error{
    BCLOG_FUNC(GATT_MODUAL)
    NSString *callback = [self getCompletionCallback:[NSString stringWithFormat:@"writeDescriptor:%d:%d:%d:%@",
                                                               [self getDescriptorIndex:descriptor.characteristic descriptor:descriptor],
                                                               [self getCharacterIndex:descriptor.characteristic.service character:descriptor.characteristic],
                                                               [self getServiceIndex:peripheral service:descriptor.characteristic.service],
                                                               [self getPeripheralUUID:peripheral]]];
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateNotificationStateForCharacteristic:
    ...
    NSString *serviceIndex = [NSString stringWithFormat:@"%d",[self getServiceIndex:peripheral service:service]];
    NSString *characteristicIndex = [NSString stringWithFormat:@"%d",[self getCharacterIndex:service character:characteristic]];
    NSString *callback = [self getCompletionCallback:[NSString stringWithFormat:@"setNotification:%@:%@:%@",
                                                               characteristicIndex,serviceIndex,deviceAddress]];
    NSString *enable = [self getCompletionCallback:[NSString stringWithFormat:@"notificationEnable:%@:%@:%@",
                                                             characteristicIndex,serviceIndex,deviceAddress]];
    // Remove notification callback
    if (([enable boolValue] && error) || (![enable boolValue] && !error))
        [self.callbacks removeObjectForKey:[NSString stringWithFormat:@"notification:%@:%@:%@",
                                                     characteristicIndex,serviceIndex,deviceAddress]];
    if (!error) {
        CDVPluginResult* result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
        [result setKeepCallbackAsBool:[enable boolValue]];
        [self.commandDelegate sendPluginResult:result callbackId:callback];
    }else{
        [self error:callback];
    }
...
- (int)getDescriptorIndex:(CBCharacteristic *)characteristic descriptor:(CBDescriptor *)descriptor{
    for (int i = 0; i < characteristic.descriptors.count; i++) {
        if ([descriptor isEqual:[characteristic.descriptors objectAtIndex:i]])
            return i;
    }
    return 0;
}

- (BOOL)operationPending:(NSString*)key command:(CDVInvokedUrlCommand*)command{
    if ([self.callbacks valueForKey:key]) {
        NSLog(@"Pending operation: %@", key);
        [self error:command.callbackId];
        return TRUE;
    }
    return FALSE;
...
- (void)removePendingOperations:(NSString*)deviceAddress{
    NSEnumerator* enumerator = [self.callbacks.allKeys objectEnumerator];
    id key;
    while ((key = [enumerator nextObject])) {
        if (![key isKindOfClass:[NSString class]])
            continue;
        if (![key containsString:deviceAddress] || [key containsString:@"disconnect:"])
            continue;
        [self.callbacks removeObjectForKey:key];
        NSLog(@"Removed pending: %@", key);
    }
}

- (NSString*)getCompletionCallback:(NSString*)key{
    NSString* callbackId = [self.callbacks valueForKey:key];
    if (callbackId) {
        [self.callbacks removeObjectForKey:key];
    } else {
        // Don't display message for readCharacteristic if there is a notification callback
        if ([key containsString:@"readCharacteristic"]) {
            NSString* ntfkey = [key stringByReplacingOccurrencesOfString:@"readCharacteristic" withString:@"notification"];
            if ([self.callbacks valueForKey:ntfkey])
                return callbackId;
        }
        NSLog(@"No callback found for %@", key);
    }
    return callbackId;
}
